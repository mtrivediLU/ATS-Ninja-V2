...
export function buildHeadline({
  ...
  if (hasRole && hasCompany) return `${greeting} Application Kit for the ${role} position at ${company} is ready. All the best!`;
  if (hasRole) return `${greeting} Application Kit for the ${role} position is ready. All the best!`;
  return `${greeting} Application Kit for this position at ${company} is ready. All the best!`;
}
 ...
 * zero.
 */
export function scoreComparisonState(original: number, tailored: number | null): ScoreComparisonState {
  ...
  const srSummary =
    tailored === null
      ...
      : `Original resume keyword match: ${original} out of 100. Tailored resume keyword match: ${tailored} out of 100. ${
  ...
  return { original, tailored, delta, srSummary, deltaLabel, noteLabel };
}
