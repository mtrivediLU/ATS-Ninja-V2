...
function StructuredResume({ document }: { document: ResumeDocument }) {
  return <>
    ...
    {document.certifications.length > 0 && <Section title="Certifications"><ul>{document.certifications.map((item, index) => <li key={`${item.name}-${index}`}>{[item.name, item.date, item.link, item.credential_id ? `Credential ID: ${item.credential_id}` : ""].filter(Boolean).join(" · ")}</li>)}</ul></Section>}
