...
import { deliveredArtifactCount, effectiveKitStatus, formatDate, kitTarget } from "@/lib/product";
...
export function HistoryWorkspace() {
  ...
  const rows = useMemo(() => {
    ...
    const current = items.filter((item) => {
      ...
      const effectiveStatus = detail ? effectiveKitStatus(detail) : item.status;
      const matchesStatus = status === "all" || status === "warnings" ? status === "all" || hasWarnings : effectiveStatus === status;
  ...
  const activeFilters = [query ? { id: "query", label: `“${query}”` } : null, status !== "all" ? { id: "status", label: status === "warnings" ? "Validation notes" : kitStatusPresentation[status].label } : null, artifact !== "all" ? { id: "artifact", label: artifactNavigation.find((item) => item.id === artifact)?.label ?? artifact } : null].filter((filter): filter is { id: string; label: string } => Boolean(filter));
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap items-center gap-3"><label className="relative min-w-[220px] flex-1"><span className="sr-only">Search loaded Kits</span><Search aria-hidden="true" className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-ink-muted" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search loaded role, company, or Kit ID…" className="pl-9" /></label><div className="flex flex-wrap gap-1" aria-label="Filter loaded Kits by status">{statusFilters.map((value) => <button key={value} type="button" aria-pressed={status === value} onClick={() => setStatus(value)} className="min-h-11 rounded-pill border border-border bg-surface px-3 text-sm text-ink-secondary aria-pressed:border-border-strong aria-pressed:bg-surface-raised aria-pressed:font-semibold aria-pressed:text-ink">{value === "all" ? "All" : value === "warnings" ? "Notes" : kitStatusPresentation[value].label}</button>)}</div><Select aria-label="Filter loaded Kits by included artifact" value={artifact} onChange={(event) => setArtifact(event.target.value as "all" | ArtifactSlug)} className="w-auto"><option value="all">Any artifact</option>{artifactNavigation.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select><Select aria-label="Sort loaded Kits" value={sort} onChange={(event) => setSort(event.target.value as Sort)} className="w-auto"><option value="recent">Newest first</option><option value="role">Role A–Z</option><option value="company">Company A–Z</option></Select></div>
...
function artifactWarning(detail: KitRead): boolean {
  const result = detail.result;
  if (!result) return false;
  const reportWarning = Object.values(result.delivery_reports ?? {}).some(
    (report) => report.state === "generated_with_fallback" || report.findings.length > 0,
  );
  return reportWarning || [result.resume, result.cover_letter, result.answers, result.job_fit, result.interview_prep, result.linkedin_outreach].some((item) => item && (item.validation.warnings.length > 0 || item.validation.status === "repaired"));
}
...
function HistoryCard({ item }: { item: SummaryLike | KitRead }) { const detail = "result" in item ? item : null; const target = kitTarget(detail); const fit = detail?.result?.job_fit?.fit_band; const artifacts = deliveredArtifactCount(detail?.result ?? null); const lifecycle = detail ? effectiveKitStatus(detail) : item.status; const schemaVersion = detail?.result?.schema_version; const oldSchema = schemaVersion !== undefined && schemaVersion !== "application-kit/v5" && schemaVersion !== "application-kit/v6" && schemaVersion !== "application-kit/v7"; const warning = detail && artifactWarning(detail); return <Link href={`/kits/${item.id}`} className="flex flex-col gap-3 rounded-md border border-border bg-surface p-4 text-ink transition-[border-color,box-shadow] hover:border-border-strong hover:shadow-xs sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><h3 className="truncate font-semibold">{target.role} — {target.company}</h3><p className="truncate font-mono text-xs text-ink-muted">{item.id}{detail?.result ? ` · ${detail.result.schema_version}` : ""}{detail?.result?.pramana_version ? ` · PRAMANA v${detail.result.pramana_version}` : ""}</p><p className="mt-1 text-xs text-ink-muted">{formatDate(item.created_at)}{fit ? ` · ${fit} fit` : ""}{artifacts ? ` · ${artifacts} artifacts` : ""}{oldSchema ? " · compatibility view" : ""}{warning ? " · validation notes" : ""}</p></div><div className="flex items-center gap-2">{oldSchema && <span className="rounded-pill border border-neutral-border bg-neutral-bg px-2 py-1 text-xs text-neutral">Older format</span>}{warning && <span className="rounded-pill border border-warning-border bg-warning-bg px-2 py-1 text-xs text-warning">Notes</span>}<StatusLabel presentation={kitStatusPresentation[lifecycle]} /></div></Link>; }
