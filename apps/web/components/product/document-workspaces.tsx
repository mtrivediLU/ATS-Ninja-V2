...
import type { AnswerArtifact, CoverLetterArtifact, DocumentState, ResumeArtifact } from "@/lib/api-types";
import { formatAnswersText } from "@/lib/artifact-content";
import { copyText, safeFilename } from "@/lib/product";

export function DocumentWorkspace({ kind, artifact, company, role, kitId, deliveryState }: { kind: "resume" | "cover-letter"; artifact: ResumeArtifact | CoverLetterArtifact; company: string; role: string; kitId: string; deliveryState?: DocumentState }) {
  ...
  const title =
    kind === "resume"
      ? deliveryState === "generated_with_fallback"
        ? "Delivered resume"
        : "Tailored resume"
      : "Cover letter";
  ...
  return <div>
    ...
    {view === "trust" ? <TrustSummary title={title} claims={artifact.claims} validation={artifact.validation} text={editor.applied} manuallyEdited={editor.edited} deliveryState={deliveryState} onOpenContent={() => setView("content")} /> : view === "template" ? <TemplatePreview artifact={kind} text={visibleText} latex={artifact.latex} company={company} role={role} edited={editor.editing || editor.edited} resumeDocument={resumeDocument} coverLetterDocument={coverLetterDocument} onReturnToArtifact={() => setView("content")} kitId={kitId} /> : <div className="mx-auto mt-5 max-w-[820px]">
...
export function AnswersWorkspace({ artifact, company, role, deliveryState }: { artifact: AnswerArtifact; company: string; role: string; deliveryState?: DocumentState }) {
  const { notify } = useFeedback();
  const [view, setView] = useArtifactView();
  ...
  return <div>
    ...
    {view === "trust" ? <TrustSummary title="Application answers" claims={artifact.claims} validation={artifact.validation} text={editor.applied} manuallyEdited={editor.edited} deliveryState={deliveryState} onOpenContent={() => setView("content")} /> : <div className="mx-auto mt-5 max-w-[860px]">
